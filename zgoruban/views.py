...
def Home(request):
    ...
    testss = TESTIMONY.objects.all()
    test = testss[:1]
    test2w = TESTIMONY.objects.all()
    test2 = test2w[1:2]
    ...
    fi2 = fiv[6:12]
    fi3 = fiv[12:18]
    d = {"fi": fi,"fi2":fi2,"fi3":fi3,"fou1":fou1,"fou2":fou2,"fou3":fou3,"thre1":thre1,"thre2":thre2,"thre3":thre3,"two1":two1,"two2":two2,"two3":two3,"test":test,"new":new,"eve":eve,"abo":abo,"cam":cam,"ana":ana,"sto":sto,"zcmap":zcmap,"test2":test2,"test":test}
...
def ADMIN_ABOUT(request):
    ...
    if request.method == "POST":
        if 'about' in request.POST:
            ...
            imgg2 = request.FILES['img2']
            imgg3 = request.FILES['img3']
            abow = About.objects.filter(id=1)
            abow.update(heading=hedd,paragraph=parr,image1=imgg,image2=imgg2,image3=imgg3)
